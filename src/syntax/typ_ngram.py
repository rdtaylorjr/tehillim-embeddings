...
def phrase_typ_1gram_psalm_vectors(psalms: list[PhrasePsalm]) -> dict[int, np.ndarray]:
    ...
    columns = [PsalmColumns(p.number, p.half_verse_nodes, p.half_verse_typ) for p in psalms]
    return pooled_ngram_psalm_vectors(columns, (1,), TYP_VOCABULARY, order_by_node=None)
...
def phrase_typ_1_2gram_psalm_vectors(
    ...
    columns = [PsalmColumns(p.number, p.half_verse_nodes, p.half_verse_typ) for p in psalms]
    return pooled_ngram_psalm_vectors(columns, (1, 2), TYP_VOCABULARY, order_by_node)
...
def phrase_typ_1_2_3gram_psalm_vectors(
    ...
    columns = [PsalmColumns(p.number, p.half_verse_nodes, p.half_verse_typ) for p in psalms]
    return pooled_ngram_psalm_vectors(columns, (1, 2, 3), TYP_VOCABULARY, order_by_node)
...
def phrase_typ_1_2_3gram_psalm_sparse_vectors(
    ...
    columns = [PsalmColumns(p.number, p.half_verse_nodes, p.half_verse_typ) for p in psalms]
    return sparse_pooled_1_2_3gram(columns, TYP_VOCABULARY, order_by_node)
