"""Conventional structural-complexity summaries per half-verse: not new inferential machinery."""
...
def half_verse_complexity_features(
    n_words: tuple[int, ...], phrase_id: tuple[int, ...], phrase_atom_count: tuple[int, ...]
) -> np.ndarray:
    """`[n_atoms, n_phrases, mean_words_per_atom, proportion_multi_atom_phrases]` for one."""
    n_atoms = len(n_words)
    if n_atoms == 0:
        return np.zeros(_N_FEATURES, dtype=np.float32)

    distinct_phrases = dict(zip(phrase_id, phrase_atom_count, strict=True))
...
def phrase_complexity_vectors(psalms: list[PhrasePsalm]) -> dict[int, np.ndarray]:
    """One complexity feature vector per half-verse node."""
    ...
    for psalm in psalms:
        for node, n_words, phrase_id, phrase_atom_count in zip(
            ...
            vectors[node] = half_verse_complexity_features(n_words, phrase_id, phrase_atom_count)
    return vectors


def phrase_complexity_psalm_vectors(psalms: list[PhrasePsalm]) -> dict[int, np.ndarray]:
    """Psalm-broadcast complexity vector: unweighted mean of every half-verse's own features."""
    vectors: dict[int, np.ndarray] = {}
    for psalm in psalms:
        half_verse_features = [
            half_verse_complexity_features(n_words, phrase_id, phrase_atom_count)
        ...
        psalm_vector = (
            np.mean(half_verse_features, axis=0)
            if half_verse_features
