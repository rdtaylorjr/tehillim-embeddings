...
def generate_shuffle_control(
    ...
    for seed in range(1, n_shuffles + 1):
        ...
        write_dataset(output_root, unit, weight, vectors, description, dataset_type=_DATASET_TYPE)
...
def generate_signature_shuffle_control(
    ...
    for seed in range(1, n_shuffles + 1):
        ...
        written.append(f"phrase_signature_{weight}")
    return written
...
def main() -> None:
    ...
    parser.add_argument(
        "--unit", required=True, choices=(*sorted(_BUILDERS_BY_UNIT), "phrase_signature")
    )
