...
def generate(psalms: list[PhrasePsalm], output_root: Path) -> list[str]:
    ...
    for weight in _FULL_WEIGHTS:
        if dataset_path(output_root, "phrase_typ", weight, dataset_type=_DATASET_TYPE).exists():
            continue
        print(f"computing phrase unit=phrase_typ construction={weight}...", file=sys.stderr)
        ...
        write_dataset(
            output_root, "phrase_typ", weight, vectors, description, dataset_type=_DATASET_TYPE
        )
        written.append(f"phrase_typ_{weight}")
    return written
