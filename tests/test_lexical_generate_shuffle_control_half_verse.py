...
from lexical.scripts.generate_shuffle_control_half_verse import _CONSTRUCTION, build_vectors
...
def _psalms():
    return [
        LexicalPsalm(
            number=1,
            half_verse_lexemes=(("A", "B"), ("A",), ("B",), ("A", "B")),
            half_verse_forms=(("A0", "B0"), ("A0",), ("B0",), ("A0", "B0")),
            half_verse_nodes=(100, 101, 102, 103),
        ),
    ]
...
class TestGenerateShuffleControlHalfVerse:
    ...
    def test_each_shuffle_gives_distinct_half_verses_distinct_vectors(self, tmp_path):
    ...
    def test_different_seeds_give_different_vectors(self, tmp_path):
        ...
        assert (
            _table(tmp_path, 1)["vector"].to_pylist() != _table(tmp_path, 2)["vector"].to_pylist()
        )

    def test_every_half_verse_node_is_written(self, tmp_path):
        _generate(tmp_path, 1)

        assert _table(tmp_path, 1)["node_id"].to_pylist() == [100, 101, 102, 103]
