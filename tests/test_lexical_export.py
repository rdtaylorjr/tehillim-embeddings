...
class TestDatasetPath:
    def test_points_at_the_hive_partitioned_parquet_file(self, tmp_path):
        path = dataset_path(tmp_path, "homograph", "binary", unit_key="unit")
        expected = (
            tmp_path
            / "domain=lexical"
            / "unit=homograph"
    ...
    def test_inserts_a_text_partition_when_given(self, tmp_path):
        path = dataset_path(tmp_path, "word", "binary", text="consonantal", unit_key="unit")
        expected = (
            tmp_path
            / "domain=lexical"
            / "unit=word"
...
class TestWriteDataset:
    def test_round_trips_node_ids_and_vectors_exactly(self, tmp_path):
        ...
        write_dataset(tmp_path, "homograph", "binary", vectors, "a description", unit_key="unit")
    ...
    def test_stores_the_description_in_file_metadata(self, tmp_path):
        ...
        write_dataset(
            tmp_path, "homograph", "binary", vectors, "a specific description", unit_key="unit"
        )
    ...
    def test_creates_parent_directories(self, tmp_path):
        ...
        write_dataset(tmp_path / "nested", "homograph", "binary", vectors, "d", unit_key="unit")

        assert dataset_path(tmp_path / "nested", "homograph", "binary", unit_key="unit").exists()

    def test_writes_to_the_text_partitioned_path_when_given(self, tmp_path):
        vectors = {100: np.array([1.0], dtype=np.float32)}

        write_dataset(tmp_path, "word", "binary", vectors, "d", text="vocalized", unit_key="unit")

        assert dataset_path(tmp_path, "word", "binary", text="vocalized", unit_key="unit").exists()
...
class TestWriteSparseDataset:
    def test_round_trips_indices_and_values_exactly(self, tmp_path):
        ...
        table = pq.read_table(
            dataset_path(tmp_path, "morph_signature", "1_2_3gram", unit_key="unit")
        )
        by_node = {
            node: (idx, val)
    ...
    def test_creates_parent_directories(self, tmp_path):
        ...
        assert dataset_path(
            tmp_path / "nested", "morph_signature", "1_2_3gram", unit_key="unit"
        ).exists()
