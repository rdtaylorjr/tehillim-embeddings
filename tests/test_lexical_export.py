...
class TestDatasetPath:
    def test_points_at_the_hive_partitioned_parquet_file(self, tmp_path):
        path = dataset_path(tmp_path, "form", "binary")
        expected = (
            tmp_path / "data" / "type=lexical" / "vocab=form" / "weight=binary" / "part-0.parquet"
        )
...
class TestWriteDataset:
    def test_round_trips_node_ids_and_vectors_exactly(self, tmp_path):
        ...
        write_dataset(tmp_path, "form", "binary", vectors, "a description")
    ...
    def test_stores_the_description_in_file_metadata(self, tmp_path):
        ...
        write_dataset(tmp_path, "form", "binary", vectors, "a specific description")
    ...
    def test_creates_parent_directories(self, tmp_path):
        ...
        write_dataset(tmp_path / "nested", "form", "binary", vectors, "d")

        assert dataset_path(tmp_path / "nested", "form", "binary").exists()
