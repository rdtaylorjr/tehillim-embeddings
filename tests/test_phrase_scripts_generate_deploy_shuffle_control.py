...
from phrase.scripts.generate_deploy_shuffle_control import generate_shuffle_control
...
class TestGenerateShuffleControl:
    def test_writes_n_seeded_datasets(self, tmp_path):
        ...
        assert written == [
            "phrase_signature_posmean_shuffle01",
            "phrase_signature_posmean_shuffle02",
            "phrase_signature_posmean_shuffle03",
        ]
        for weight in ("posmean_shuffle01", "posmean_shuffle02", "posmean_shuffle03"):
            assert dataset_path(tmp_path, "phrase_signature", weight).exists()
    ...
    def test_each_shuffle_broadcasts_the_same_vector_to_every_colon(self, tmp_path):
        ...
        table = pq.read_table(dataset_path(tmp_path, "phrase_signature", "posmean_shuffle01"))
    ...
    def test_different_seeds_give_different_vectors(self, tmp_path):
        ...
        table1 = pq.read_table(dataset_path(tmp_path, "phrase_signature", "posmean_shuffle01"))
        table2 = pq.read_table(dataset_path(tmp_path, "phrase_signature", "posmean_shuffle02"))
