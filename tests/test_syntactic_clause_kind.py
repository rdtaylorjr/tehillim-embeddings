...
class TestClauseKindColumns:
    def test_one_sequence_per_half_verse_in_node_order(self):
...
class TestClauseKindInventoryVectors:
    def test_a_half_verse_histogram_is_the_proportion_of_each_kind(self):
    ...
    def test_a_histogram_sums_to_one_when_the_half_verse_has_a_clause(self):
        psalm = _psalm(1, (10,), ("WP",), [0], [0], [1.0])

        assert clause_kind_1gram_vectors([psalm])[10].sum() == pytest.approx(1.0)

    def test_a_half_verse_no_clause_reaches_is_the_zero_vector(self):
...
class TestClauseKindInventoryPsalmVectors:
    def test_every_half_verse_of_a_psalm_shares_one_pooled_vector(self):
    ...
    def test_pooling_counts_every_clause_in_the_psalm_not_just_one_half_verse(self):
