...
def _psalm(*, number, phrase_rela_by_half_verse, nodes=None):
    nodes = nodes if nodes is not None else tuple(range(100, 100 + len(phrase_rela_by_half_verse)))
    return PhrasePsalm(
        number=number, half_verse_nodes=nodes, half_verse_subphrase_rela=phrase_rela_by_half_verse
    )
...
class TestSubphraseRela1gramVectors:
    def test_has_dimension_of_the_safe_vocabulary(self):
        psalms = [_psalm(number=1, phrase_rela_by_half_verse=(("NA", "rec"),))]
    ...
    def test_keys_vectors_by_half_verse_node_id(self):
        psalms = [_psalm(number=1, phrase_rela_by_half_verse=(("NA",), ("rec",)), nodes=(200, 201))]
        vectors = subphrase_rela_1gram_vectors(psalms)
...
class TestSubphraseRela1gramPsalmVectors:
    def test_broadcasts_the_identical_vector_to_every_half_verse_node(self):
        psalms = [_psalm(number=1, phrase_rela_by_half_verse=(("NA",), ("rec",)), nodes=(400, 401))]
        vectors = subphrase_rela_1gram_psalm_vectors(psalms)
        assert np.array_equal(vectors[400], vectors[401])

    def test_masks_par_before_pooling_across_half_verses(self):
        psalms = [
            _psalm(number=1, phrase_rela_by_half_verse=(("par",), ("rec",)), nodes=(500, 501))
        ]
