...
from phrase.generate_typ import generate
...
def _psalm(*, number, typ_by_colon, nodes):
    return PhrasePsalm(number=number, half_verse_nodes=nodes, half_verse_typ=typ_by_colon)


def _psalms():
    return [
        _psalm(number=1, typ_by_colon=(("NP", "VP"), ("PP",)), nodes=(100, 101)),
        _psalm(number=2, typ_by_colon=(("CP",),), nodes=(200,)),
    ]
...
class TestGenerate:
    def test_writes_the_full_phrase_typ_representation_family(self, tmp_path):
        ...
        assert set(written) == {f"phrase_typ_{w}" for w in _FULL_WEIGHTS}
        for weight in _FULL_WEIGHTS:
            assert dataset_path(tmp_path, "phrase_typ", weight).exists()
    ...
    def test_psalm_variants_broadcast_the_same_vector_within_a_psalm(self, tmp_path):
        ...
        table = pq.read_table(dataset_path(tmp_path, "phrase_typ", "1gram_psalm"))
    ...
    def test_colon_level_variants_give_distinct_colons_their_own_vector(self, tmp_path):
        ...
        table = pq.read_table(dataset_path(tmp_path, "phrase_typ", "1gram"))
        by_node = dict(zip(table["node_id"].to_pylist(), table["vector"].to_pylist(), strict=True))
        assert by_node[100] != by_node[101]
