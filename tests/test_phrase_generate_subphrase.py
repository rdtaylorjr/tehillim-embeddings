...
from phrase.generate_subphrase import generate
...
class TestGenerate:
    def test_writes_both_constructions(self, tmp_path):
        ...
        assert set(written) == {"phrase_subphrase_rela_1gram", "phrase_subphrase_rela_1gram_psalm"}
        assert dataset_path(tmp_path, "phrase_subphrase_rela", "1gram").exists()
        assert dataset_path(tmp_path, "phrase_subphrase_rela", "1gram_psalm").exists()
    ...
    def test_written_vectors_never_show_mass_outside_the_safe_vocabulary_dimension(self, tmp_path):
        ...
        table = pq.read_table(dataset_path(tmp_path, "phrase_subphrase_rela", "1gram"))
