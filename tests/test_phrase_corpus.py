...
def test_extracts_all_150_psalms_with_phrase_typ_aligned_to_colons():
    ...
    for typ_colon, *rest in zip(*feature_sequences, strict=True):
        for colon in rest:
            assert len(colon) == len(typ_colon)
...
@pytest.mark.integration
def test_a_real_colons_typ_sequence_matches_a_manual_tf_query():
    corpus = Corpus.load()
...
@pytest.mark.integration
def test_typ_values_match_the_verified_thirteen_value_bhsa_inventory():
...
@pytest.mark.integration
def test_a_real_colons_function_sequence_matches_a_manual_tf_query():
    corpus = Corpus.load()
    psalm_1 = next(p for p in corpus.psalms() if p.number == 1)
    api = corpus.api
    colon_node = psalm_1.half_verse_nodes[0]
...
@pytest.mark.integration
def test_function_values_observed_in_psalms_are_a_subset_of_the_frozen_inventory():
    from phrase.vocabulary import FUNCTION_VOCABULARY
...
@pytest.mark.integration
def test_det_n_words_and_phrase_bookkeeping_match_a_manual_tf_query():
    corpus = Corpus.load()
    psalm_1 = next(p for p in corpus.psalms() if p.number == 1)
    api = corpus.api
...
@pytest.mark.integration
def test_rela_matches_a_manual_tf_query_and_whole_bhsa_inventory_includes_para():
    corpus = Corpus.load()
    psalm_1 = next(p for p in corpus.psalms() if p.number == 1)
    api = corpus.api
    colon_node = psalm_1.half_verse_nodes[0]
...
def test_load_accepts_an_explicit_path():
    from phrase.corpus import DEFAULT_BHSA_TF_PATH
