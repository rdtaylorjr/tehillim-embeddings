...
from syntactic.generate_typ import _vectors_for_weight, generate
...
def _psalm(*, number, phrase_typ_by_half_verse, nodes):
    return PhrasePsalm(
...
def _psalms():
    return [
        _psalm(number=1, phrase_typ_by_half_verse=(("NP", "VP"), ("PP",)), nodes=(100, 101)),
        _psalm(number=2, phrase_typ_by_half_verse=(("CP",),), nodes=(200,)),
    ]
...
def test_an_unregistered_weight_is_rejected_rather_than_silently_skipped() -> None:
    """A construction name with no builder is a config error, not an empty phrase-type dataset."""
