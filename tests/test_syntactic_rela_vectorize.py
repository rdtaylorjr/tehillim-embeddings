...
def _psalm(*, number, phrase_rela_by_half_verse, nodes=None):
    ...
    return PhrasePsalm(
        number=number, half_verse_nodes=nodes, half_verse_rela=phrase_rela_by_half_verse
    )
...
class TestPhraseRela1gramVectors:
    def test_has_dimension_of_the_safe_vocabulary(self):
        psalms = [_psalm(number=1, phrase_rela_by_half_verse=(("NA", "Appo"),))]
    ...
    def test_keys_vectors_by_half_verse_node_id(self):
        psalms = [
            _psalm(number=1, phrase_rela_by_half_verse=(("NA",), ("Appo",)), nodes=(200, 201))
        ]
        vectors = phrase_rela_1gram_vectors(psalms)
...
class TestPhraseRela1gramPsalmVectors:
    def test_broadcasts_the_identical_vector_to_every_half_verse_node(self):
        psalms = [
            _psalm(number=1, phrase_rela_by_half_verse=(("NA",), ("Appo",)), nodes=(400, 401))
        ]
        vectors = phrase_rela_1gram_psalm_vectors(psalms)
        assert np.array_equal(vectors[400], vectors[401])

    def test_masks_para_before_pooling_across_half_verses(self):
        psalms = [
            _psalm(number=1, phrase_rela_by_half_verse=(("Para",), ("Appo",)), nodes=(500, 501))
        ]
