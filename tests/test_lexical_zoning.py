from __future__ import annotations

import numpy as np

from lexical.corpus import LexicalPsalm
from lexical.vocabulary import columns_for_key
from lexical.zoning import position_mean_vectors


def _psalm(*, number, lexemes, forms, nodes):
    return LexicalPsalm(
        number=number,
        half_verse_lexemes=lexemes,
        half_verse_forms=forms,
        half_verse_nodes=nodes,
    )


class TestPositionalCentroidVectors:
    def test_vector_length_is_twice_the_vocabulary_size(self):
        vocabulary = ("A", "B", "C")
        icf_weights = {"A": 1.0, "B": 1.0, "C": 1.0}
        psalms = [_psalm(number=1, lexemes=(("A",), ("B",)), forms=((), ()), nodes=(100, 101))]

        vectors = position_mean_vectors(
            columns_for_key(psalms, "lex"), vocabulary, icf_weights=icf_weights
        )

        assert len(vectors[100]) == 6

    def test_inventory_half_is_icf_weight_if_present_in_that_half_verse(self):
        vocabulary = ("A", "B", "C")
        icf_weights = {"A": 2.0, "B": 3.0, "C": 5.0}
        # A appears in every half-verse, B only in half-verse 1, C absent.
        psalms = [
            _psalm(
                number=1,
                lexemes=(("A",), ("A", "B"), ("A",)),
                forms=((), (), ()),
                nodes=(100, 101, 102),
            )
        ]

        vectors = position_mean_vectors(
            columns_for_key(psalms, "lex"), vocabulary, icf_weights=icf_weights
        )

        # half-verse 0 has only A: b reflects this half-verse's own content, not the whole psalm's.
        assert np.allclose(vectors[100][:3], [2.0, 0.0, 0.0])
        assert np.allclose(vectors[101][:3], [2.0, 3.0, 0.0])

    def test_m_equals_the_half_verses_own_position_scaled_by_icf_weight(self):
        vocabulary = ("A",)
        icf_weights = {"A": 4.0}
        # 4 half-verses, A only in half-verse index 0 -> t_0 = (1-0.5)/4 = 0.125
        psalms = [
            _psalm(
                number=1,
                lexemes=(("A",), (), (), ()),
                forms=((), (), (), ()),
                nodes=(100, 101, 102, 103),
            )
        ]

        vectors = position_mean_vectors(
            columns_for_key(psalms, "lex"), vocabulary, icf_weights=icf_weights
        )

        m = vectors[100][1]
        expected_t = 0.125
        assert np.isclose(m, 4.0 * (2 * expected_t - 1))

    def test_the_same_lexeme_gets_a_different_m_per_half_verse_matching_each_ones_own_position(
        self,
    ):
        vocabulary = ("A",)
        icf_weights = {"A": 4.0}
        psalms = [
            _psalm(
                number=1,
                lexemes=(("A",), ("A",), ("A",), ("A",)),
                forms=((), (), (), ()),
                nodes=(100, 101, 102, 103),
            )
        ]

        vectors = position_mean_vectors(
            columns_for_key(psalms, "lex"), vocabulary, icf_weights=icf_weights
        )

        # symmetric half-verse positions -> symmetric signed m values, distinct per half-verse.
        assert vectors[100][1] < 0.0
        assert vectors[103][1] > 0.0
        assert not np.isclose(vectors[100][1], vectors[101][1])

    def test_early_leaning_lexeme_has_negative_m_and_late_leaning_has_positive_m(self):
        vocabulary = ("EARLY", "LATE")
        icf_weights = {"EARLY": 1.0, "LATE": 1.0}
        # 4 half-verses: EARLY only in half-verse 0, LATE only in half-verse 3.
        psalms = [
            _psalm(
                number=1,
                lexemes=(("EARLY",), (), (), ("LATE",)),
                forms=((), (), (), ()),
                nodes=(100, 101, 102, 103),
            )
        ]

        vectors = position_mean_vectors(
            columns_for_key(psalms, "lex"), vocabulary, icf_weights=icf_weights
        )

        m_early = vectors[100][2]
        m_late = vectors[103][3]
        assert m_early < 0
        assert m_late > 0

    def test_absent_lexeme_is_zero_in_both_halves(self):
        vocabulary = ("A", "B")
        icf_weights = {"A": 1.0, "B": 99.0}
        psalms = [_psalm(number=1, lexemes=(("A",),), forms=((),), nodes=(100,))]

        vectors = position_mean_vectors(
            columns_for_key(psalms, "lex"), vocabulary, icf_weights=icf_weights
        )

        assert vectors[100][1] == 0.0  # b for B
        assert vectors[100][3] == 0.0  # m for B

    def test_each_half_verse_gets_its_own_distinguishable_vector(self):
        vocabulary = ("A", "B")
        icf_weights = {"A": 1.0, "B": 1.0}
        psalms = [
            _psalm(
                number=1,
                lexemes=(("A",), ("B",), ("A",)),
                forms=((), (), ()),
                nodes=(100, 101, 102),
            )
        ]

        vectors = position_mean_vectors(
            columns_for_key(psalms, "lex"), vocabulary, icf_weights=icf_weights
        )

        assert not np.array_equal(vectors[100], vectors[101])
        # half-verse 0 and half-verse 2 share content (A) but sit at different positions.
        assert not np.array_equal(vectors[100], vectors[102])

    def test_inventory_half_is_invariant_under_shuffled_order_but_position_half_is_not(self):
        vocabulary = ("A",)
        icf_weights = {"A": 4.0}
        psalms = [
            _psalm(
                number=1,
                lexemes=(("A",), (), (), ()),
                forms=((), (), (), ()),
                nodes=(100, 101, 102, 103),
            )
        ]

        natural = position_mean_vectors(
            columns_for_key(psalms, "lex"), vocabulary, icf_weights=icf_weights
        )
        shuffled = position_mean_vectors(
            columns_for_key(psalms, "lex"),
            vocabulary,
            icf_weights=icf_weights,
            order_by_psalm={1: np.array([3, 1, 2, 0])},  # moves half-verse 0 to position 3
        )

        assert natural[100][0] == shuffled[100][0]  # b: invariant, same half-verse's own content
        assert natural[100][1] != shuffled[100][1]  # m: not invariant, half-verse's position moved

    def test_uses_lex0_forms_when_key_is_lex0(self):
        vocabulary = ("A0",)
        icf_weights = {"A0": 1.0}
        psalms = [_psalm(number=1, lexemes=(("A",),), forms=(("A0",),), nodes=(100,))]

        vectors = position_mean_vectors(
            columns_for_key(psalms, "lex0"), vocabulary, icf_weights=icf_weights
        )

        assert len(vectors[100]) == 2
