...
def _psalm(*, number, phrase_typ_by_half_verse, nodes=None):
    nodes = nodes if nodes is not None else tuple(range(100, 100 + len(phrase_typ_by_half_verse)))
    return PhrasePsalm(
        number=number, half_verse_nodes=nodes, half_verse_typ=phrase_typ_by_half_verse
    )
...
class TestPhraseTypBigramHistogram:
    ...
    def test_sums_to_one_for_a_half_verse_with_at_least_two_atoms(self):
...
class TestPhraseTypTrigramHistogram:
    ...
    def test_sums_to_one_for_a_half_verse_with_at_least_three_atoms(self):
...
class TestPhraseTypNgramVectors:
    def test_1gram_vectors_has_dimension_of_the_vocabulary(self):
        psalms = [_psalm(number=1, phrase_typ_by_half_verse=(("NP", "VP"),))]
    ...
    def test_keys_vectors_by_half_verse_node_id(self):
        psalms = [_psalm(number=1, phrase_typ_by_half_verse=(("NP",), ("VP",)), nodes=(200, 201))]
        vectors = phrase_typ_1gram_vectors(psalms)
        assert set(vectors) == {200, 201}

    def test_order_by_node_reorders_atoms_within_that_half_verse_only(self):
        psalms = [_psalm(number=1, phrase_typ_by_half_verse=(("NP", "VP", "PP"),), nodes=(300,))]
...
class TestPhraseTypNgramPsalmVectors:
    def test_broadcasts_the_identical_vector_to_every_half_verse_node(self):
        psalms = [_psalm(number=1, phrase_typ_by_half_verse=(("NP",), ("VP",)), nodes=(400, 401))]
    ...
    def test_pools_raw_atom_counts_across_half_verses_before_normalizing_once(self):
        psalms = [
            _psalm(
                number=1, phrase_typ_by_half_verse=(("NP",), ("VP", "VP", "PP")), nodes=(500, 501)
            )
        ]
    ...
    def test_order_by_node_reorders_each_half_verses_atoms_before_pooling(self):
        psalms = [_psalm(number=1, phrase_typ_by_half_verse=(("NP", "VP", "PP"),), nodes=(600,))]
