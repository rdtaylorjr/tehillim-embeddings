...
class TestBinaryPresenceVectors:
    def test_marks_present_lexemes_as_one_and_absent_as_zero(self):
        ...
        vectors = binary_presence_vectors(columns_for_key(psalms, "lex"), vocabulary)

        assert np.array_equal(vectors[100], [1.0, 0.0, 1.0])

    def test_a_lexeme_repeated_within_a_half_verse_still_scores_one_not_two(self):
        vocabulary = ("A", "B")
    ...
    def test_uses_lex0_forms_when_key_is_lex0(self):
        ...
        vectors = binary_presence_vectors(columns_for_key(psalms, "lex0"), vocabulary)
    ...
    def test_matches_thresholding_term_frequency_vectors_exactly(self):
        ...
        binary = binary_presence_vectors(columns_for_key(psalms, "lex"), vocabulary)
        counts = term_frequency_vectors(columns_for_key(psalms, "lex"), vocabulary)
...
class TestTermFrequencyVectors:
    def test_counts_repeated_occurrences_within_a_half_verse(self):
        vocabulary = ("A", "B")
        psalms = [_psalm(number=1, lexemes=(("A", "A", "A", "B"),), forms=((),), nodes=(100,))]

        vectors = term_frequency_vectors(columns_for_key(psalms, "lex"), vocabulary)
...
class TestLogCountVectors:
    def test_matches_log1p_of_the_raw_count(self):
        ...
        vectors = log_count_vectors(columns_for_key(psalms, "lex"), vocabulary)
