from __future__ import annotations

import pyarrow.parquet as pq

from core.partition import BHSA_HALF_VERSE, Partition
from syntactic.corpus import PhrasePsalm
from syntactic.generate_det import generate


def dataset_path(output_root, vocab, weight):
    partition = Partition(
        BHSA_HALF_VERSE, "syntactic", level="phrase", feature=vocab, construction=weight
    )
    return partition.file(output_root)


def _psalm(*, number, nodes, det):
    return PhrasePsalm(number=number, half_verse_nodes=nodes, half_verse_det=det)


def _psalms():
    return [_psalm(number=1, nodes=(100, 101), det=(("det", "und"), ("NA",)))]


class TestGenerate:
    def test_writes_every_declared_construction(self, tmp_path):
        written = generate(_psalms(), tmp_path)

        assert set(written) == {
            "phrase_det_1gram",
            "phrase_det_1gram_psalm",
            "phrase_det_1_2gram",
            "phrase_det_1_2gram_psalm",
            "phrase_det_1_2_3gram",
            "phrase_det_1_2_3gram_psalm",
        }
        assert dataset_path(tmp_path, "det", "1gram").exists()
        assert dataset_path(tmp_path, "det", "1gram_psalm").exists()
        assert dataset_path(tmp_path, "det", "1_2gram").exists()
        assert dataset_path(tmp_path, "det", "1_2gram_psalm").exists()
        assert dataset_path(tmp_path, "det", "1_2_3gram").exists()
        assert dataset_path(tmp_path, "det", "1_2_3gram_psalm").exists()

    def test_skips_variants_whose_dataset_already_exists(self, tmp_path):
        generate(_psalms(), tmp_path)

        written_again = generate(_psalms(), tmp_path)

        assert written_again == []

    def test_psalm_variant_broadcasts_the_same_vector(self, tmp_path):
        generate(_psalms(), tmp_path)

        table = pq.read_table(dataset_path(tmp_path, "det", "1gram_psalm"))
        by_node = dict(zip(table["node_id"].to_pylist(), table["vector"].to_pylist(), strict=True))
        assert by_node[100] == by_node[101]
