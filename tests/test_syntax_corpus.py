...
def test_extracts_all_150_psalms_with_phrase_typ_aligned_to_half_verses():
    ...
    assert len(psalm_1.half_verse_typ) == 14
    assert len(psalm_1.half_verse_function) == 14
    feature_sequences = (
        psalm_1.half_verse_typ,
        psalm_1.half_verse_function,
        psalm_1.half_verse_det,
        psalm_1.half_verse_rela,
        psalm_1.half_verse_n_words,
        psalm_1.half_verse_phrase_id,
        psalm_1.half_verse_phrase_atom_count,
    )
    for phrase_typ_half_verse, *rest in zip(*feature_sequences, strict=True):
        for half_verse in rest:
            assert len(half_verse) == len(phrase_typ_half_verse)

    # subphrase_rela is indexed by subphrase, not phrase atom, so only half-verse counts align.
    assert len(psalm_1.half_verse_subphrase_rela) == 14
...
def test_a_real_half_verses_phrase_typ_sequence_matches_a_manual_tf_query():
    ...
    manual = tuple(api.F.typ.v(pa) for pa in api.L.d(half_verse_node, otype="phrase_atom"))

    assert psalm_1.half_verse_typ[0] == manual
...
def test_a_real_half_verses_phrase_function_sequence_matches_a_manual_tf_query():
    ...
    manual = tuple(
        ...
        for pa in api.L.d(half_verse_node, otype="phrase_atom")
    )

    assert psalm_1.half_verse_function[0] == manual
...
def test_phrase_det_n_words_and_phrase_bookkeeping_match_a_manual_tf_query():
    ...
    half_verse_node = psalm_1.half_verse_nodes[0]
    atoms = L.d(half_verse_node, otype="phrase_atom")
    ...
    assert psalm_1.half_verse_det[0] == manual_det
    assert psalm_1.half_verse_n_words[0] == manual_n_words
    assert psalm_1.half_verse_phrase_id[0] == manual_phrase_id
    assert psalm_1.half_verse_phrase_atom_count[0] == manual_atom_count
...
def test_phrase_rela_matches_a_manual_tf_query_and_whole_bhsa_inventory_includes_para():
    ...
    manual_rela = tuple(api.F.rela.v(pa) for pa in api.L.d(half_verse_node, otype="phrase_atom"))
    assert psalm_1.half_verse_rela[0] == manual_rela
...
def test_subphrase_rela_matches_a_manual_tf_query_and_whole_bhsa_inventory_includes_par():
    ...
    manual_rela = tuple(api.F.rela.v(sp) for sp in api.L.d(half_verse_node, otype="subphrase"))
    assert psalm_1.half_verse_subphrase_rela[0] == manual_rela

    # Same risk as phrase_atom's rela=Para, but lowercase and far more common (417 vs 15 in Psalms).
